...
export function Description() {
  return (
    <MathJaxContext key='description'>
      ...
      <p>
        ...
        Well, we know that {`\\(2^2 = 4 \\)`}, so 2 is too little, but {`\\(3^2 = 9 \\)`}, so 3 is too much. So the number must be something between 2 and three. Let’s try 2.5:
      </p>
      {`\\(2.5^2 = 6.25 \\)`}
      <p>It’s still too much (by 1.25), so maybe let’s try a bit less:</p>
      {`\\(2.2^2 = 4.84 \\)`}
      <p>
      ...
      <p>
        ...
        If you take a number, perform some calculation on it, take the result, perform exactly the same calculation on it, then take the new results and perform this calculation again, the set of all the results will be the orbit of your number under whatever calculation you did.In our example, we had an initial numberof 2.5, a function f(x) = x – ((x ^ 2 - c) / (2x)), and we got: 2.5, 2.236(1), and 2.236067977915804(and the next numbers would be….).In other words, the orbit of 2.5 under x – ((x ^ 2 - c) / (2x)) is 2.5, 2.236(1), 2.236067977915804 ….
      ...
      <p>
        ...
        We know that sqrt(1) = 1 or - 1 – but how much is sqrt(-1) ? Someone might have told you can’t have a square root of a negative number, but this is not exactly true.There is a square root of - 1, it’s just not a real number.It’s an imaginary number, and is denoted by i.There isn’t much more to I – you just need to accept that I = sqrt(-1), and while you can eat three apples, you can’t eat i apples because imaginary numbers don’t make any sense in everyday experience.
        I is also used as a basic imaginary unit, so when sqrt(4) = 2 or - 2, sqrt(-4) = 2i or - 2i.
        But not all numbers are either real(the “normal numbers” familiar from real life), or imaginary.There are numbers that have both real and imaginary part: for example in 4 – 7i, 4 is the real part, and - 7i is an imaginary part.Many high degree equations, which seem to not have solutions, actually have solutions which are complex numbers.
      ...
      <h3>OK, so what is that picture about ?</h3>
      <p>
