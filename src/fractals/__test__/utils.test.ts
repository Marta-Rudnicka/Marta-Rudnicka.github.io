...
describe('getDistance', () => {
  it('can calculate distance between two points on the x axis', () => {
    const a: Point = [3, 0];
    const b: Point = [10, 0];
    expect(getDistance(a, b)).toBe(7);
  });
  it('can calculate distance between two points on the x axis given in reverse order', () => {
    const a: Point = [3, 0];
    const b: Point = [10, 0];
    expect(getDistance(b, a)).toBe(7);
  });
  it('can calculate distance between two points on the y axis', () => {
    const a: Point = [0, 2];
    const b: Point = [0, -8];
    expect(getDistance(a, b)).toBe(10);
  });
  it('can calculate distance between two points on the y axis given in reverse order', () => {
    const a: Point = [0, 2];
    const b: Point = [0, -8];
    expect(getDistance(b, a)).toBe(10);
  });
  it('can calculate distance between two points in the plane', () => {
    const a: Point = [2, 4];
    const b: Point = [6, 1];
    expect(getDistance(a, b)).toBe(5);
  });
  it('can calculate distance between two points in the plane in a different order', () => {
    const a: Point = [2, 4];
    const b: Point = [6, 1];
    expect(getDistance(b, a)).toBe(5);
  });
})

describe('getInitialPixelMap', () => {
  it('should return pixel map', () => {
    const expectedMap = {
      '1': {
        '1': null,
        '2': null,
    ...
    expect(getInitialPixelMap(3)).toStrictEqual(expectedMap);
  });
});
describe('mapPixelsToComplexPlaneMap', () => {
  it('should return complex numbers mapped to pixels (easy case)', () => {
    const map = {
      '1': {
        '1': null,
        '2': null,
    ...
    const numbers = [ complex(0, 0), complex(0, 0.3), complex(0, 0.6), 
      complex(0.3, 0), complex(0.3, 0.3), complex(0.3, 0.6),
      complex(0.6, 0), complex(0.6, 0.3),complex(0.6, 0.6)
    ];

    expect(mapPixelsToComplexPlane(map, 3, complex(0, 0), 0.6).complexPlane).toStrictEqual(expected);
    expect(mapPixelsToComplexPlane(map, 3, complex(0, 0), 0.6).mappedNumbers).toStrictEqual(numbers);
  });
});
